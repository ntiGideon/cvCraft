...
type Skill struct {
	...
	Level skill.Level `json:"level,omitempty"`
...
func (*Skill) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case skill.FieldName, skill.FieldCategory, skill.FieldLevel:
	...
	for i := range columns {
		...
		case skill.FieldLevel:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				s.Level = skill.Level(value.String)
			}
		default:
...
func (s *Skill) QueryResume() *ResumeQuery {
	return NewSkillClient(s.config).QueryResume(s)
}
