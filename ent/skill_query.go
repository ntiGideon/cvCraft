...
func (sq *SkillQuery) QueryResume() *ResumeQuery {
	query := (&ResumeClient{config: sq.config}).Query()
...
func (sq *SkillQuery) Clone() *SkillQuery {
	...
	return &SkillQuery{
		...
		withResume: sq.withResume.Clone(),
...
func (sq *SkillQuery) WithResume(opts ...func(*ResumeQuery)) *SkillQuery {
	query := (&ResumeClient{config: sq.config}).Query()
...
func (sq *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := sq.withResume; query != nil {
		if err := sq.loadResume(ctx, query, nodes,
			func(n *Skill) { n.Edges.Resume = []*Resume{} },
			func(n *Skill, e *Resume) { n.Edges.Resume = append(n.Edges.Resume, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SkillQuery) loadResume(ctx context.Context, query *ResumeQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Resume)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(skill.ResumeTable)
		s.Join(joinT).On(s.C(resume.FieldID), joinT.C(skill.ResumePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(skill.ResumePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(skill.ResumePrimaryKey[1]))
