...
func (cq *CertificationQuery) QueryResume() *ResumeQuery {
	query := (&ResumeClient{config: cq.config}).Query()
...
func (cq *CertificationQuery) Clone() *CertificationQuery {
	...
	return &CertificationQuery{
		...
		withResume: cq.withResume.Clone(),
...
func (cq *CertificationQuery) WithResume(opts ...func(*ResumeQuery)) *CertificationQuery {
	query := (&ResumeClient{config: cq.config}).Query()
...
func (cq *CertificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Certification, error) {
	...
	if query := cq.withResume; query != nil {
		if err := cq.loadResume(ctx, query, nodes,
			func(n *Certification) { n.Edges.Resume = []*Resume{} },
			func(n *Certification, e *Resume) { n.Edges.Resume = append(n.Edges.Resume, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CertificationQuery) loadResume(ctx context.Context, query *ResumeQuery, nodes []*Certification, init func(*Certification), assign func(*Certification, *Resume)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Certification)
	nids := make(map[int]map[*Certification]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(certification.ResumeTable)
		s.Join(joinT).On(s.C(resume.FieldID), joinT.C(certification.ResumePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(certification.ResumePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(certification.ResumePrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Certification]struct{}{byID[outValue]: {}}
