...
func (pq *ProjectQuery) QueryResume() *ResumeQuery {
	query := (&ResumeClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withResume: pq.withResume.Clone(),
...
func (pq *ProjectQuery) WithResume(opts ...func(*ResumeQuery)) *ProjectQuery {
	query := (&ResumeClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withResume; query != nil {
		if err := pq.loadResume(ctx, query, nodes,
			func(n *Project) { n.Edges.Resume = []*Resume{} },
			func(n *Project, e *Resume) { n.Edges.Resume = append(n.Edges.Resume, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadResume(ctx context.Context, query *ResumeQuery, nodes []*Project, init func(*Project), assign func(*Project, *Resume)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(project.ResumeTable)
		s.Join(joinT).On(s.C(resume.FieldID), joinT.C(project.ResumePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(project.ResumePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(project.ResumePrimaryKey[1]))
