...
func (tq *TemplateQuery) QueryOwner() *UserQuery {
...
func (tq *TemplateQuery) QueryResumes() *ResumeQuery {
	query := (&ResumeClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TemplateQuery) Clone() *TemplateQuery {
	...
	return &TemplateQuery{
		...
		withResumes: tq.withResumes.Clone(),
...
func (tq *TemplateQuery) WithOwner(opts ...func(*UserQuery)) *TemplateQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TemplateQuery) WithResumes(opts ...func(*ResumeQuery)) *TemplateQuery {
	query := (&ResumeClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withResumes = query
	return tq
}
...
func (tq *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	...
	if query := tq.withOwner; query != nil {
		if err := tq.loadOwner(ctx, query, nodes, nil,
			func(n *Template, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withResumes; query != nil {
		if err := tq.loadResumes(ctx, query, nodes,
...
func (tq *TemplateQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Template, init func(*Template), assign func(*Template, *User)) error {
...
func (tq *TemplateQuery) loadResumes(ctx context.Context, query *ResumeQuery, nodes []*Template, init func(*Template), assign func(*Template, *Resume)) error {
	...
	for _, n := range neighbors {
		fk := n.template_resumes
		if fk == nil {
			return fmt.Errorf(`foreign-key "template_resumes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "template_resumes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
